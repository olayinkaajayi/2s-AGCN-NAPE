...
class Feeder(Dataset):
    def __init__(self, data_path, label_path=None, p_interval=1, split='train', random_choose=False, random_shift=False,
                 ...
                 vel=False, sort=False, repeat=True):
        """
        :param data_path:
        :param label_path:
    ...
    def __getitem__(self, index):
        ...
        data_numpy = tools.valid_crop_resize(data_numpy, valid_frame_num, self.p_interval, self.window_size, rrot=self.random_rot) # shape: C,T,V,M
        if self.random_rot and (self.window_size != -1):
            data_numpy = tools.random_rot(data_numpy) # shape: C,T,V,M
        if self.vel:
            data_numpy[:, :-1] = data_numpy[:, 1:] - data_numpy[:, :-1]
            data_numpy[:, -1] = 0

        data_numpy = data_numpy.permute(1, 3, 2, 0).contiguous() # shape: T,M,V,C

        # classes involving two persons
        two_persons = list(range(49,60))+list(range(105,120))
        if (label not in two_persons) and self.repeat:
            self.repeat_skeleton_for_one_body(data_numpy,label)

        T, _, _, C = data_numpy.shape

        return data_numpy.view(T,-1,C), label, index
    ...
    def repeat_skeleton_for_one_body(self, data, label):
        """
